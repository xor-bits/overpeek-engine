...
namespace oe::utils {
	...
	public:
		...
		[[nodiscard]] T randomf(T min = std::numeric_limits<T>().min(), T max = std::numeric_limits<T>().max()) noexcept
		{
			std::uniform_real_distribution<T> dist(min, max);
			return dist(gen);
		}
		template<typename T = float, typename = std::enable_if_t<std::is_floating_point_v<T>>>
		[[nodiscard]] T normalf(T mean = 0.0, T deviation = 1.0) noexcept
		{
			std::normal_distribution<T> dist(mean, deviation);
			return dist(gen);
		}
		template<typename T = int, typename = std::enable_if_t<std::is_integral_v<T>>>
		[[nodiscard]] T randomi(T min = std::numeric_limits<T>().min(), T max = std::numeric_limits<T>().max()) noexcept
		{
			std::uniform_int_distribution<T> dist(min, max);
			return dist(gen);
		}

		[[nodiscard]] glm::vec2 randomVec2(float min, float max) noexcept;
		[[nodiscard]] glm::vec3 randomVec3(float min, float max) noexcept;
		[[nodiscard]] glm::vec4 randomVec4(float min, float max) noexcept;
	};
