...
namespace oe::graphics {
	...
	void(*m_key_callback)(int key, int action, int mods) = nullptr;
	void(*m_button_callback)(int button, int action) = nullptr;
	void(*m_scroll_callback)(double delta) = nullptr;
	void(*m_resize_callback)(const glm::vec2& framebuffer_size) = nullptr;
	void(*m_charmods_callback)(unsigned int codepoint, int mods) = nullptr;
	void(*m_cursor_position_callback)(const glm::vec2& transformed, const glm::vec2& window) = nullptr;
	...
	void Window::setKeyboardCallback(void (*callback)(int key, int action, int mods)) { m_key_callback = callback; }
	void Window::setButtonCallback(void(*callback)(int button, int action)) { m_button_callback = callback; }
	void Window::setScrollCallback(void(*callback)(double delta)) { m_scroll_callback = callback; }
	void Window::setResizeCallback(void(*callback)(const glm::vec2& framebuffer_size)) { m_resize_callback = callback; }
	void Window::setCharmodCallback(void(*callback)(unsigned int codepoint, int mods)) { m_charmods_callback = callback; }
	void Window::setCursorPositionCallback(void(*callback)(const glm::vec2& transformed, const glm::vec2& window)) { m_cursor_position_callback = callback; }
	...
	void key_callback(int key, int scancode, int action, int mods) {
		...
		if (m_key_callback) (*m_key_callback)(key, action, mods);
	}
