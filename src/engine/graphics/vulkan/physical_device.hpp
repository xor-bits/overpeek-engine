#pragma once

#include "../config.hpp"
#include "window.hpp"

#include <vulkan/vulkan.hpp>

#include <map>
#include <optional>
#include <set>



namespace oe::vulkan {

	class PhysicalDevice {
	public:
		struct QueueFamilyIndices {
			std::optional<uint32_t> graphicsFamily;
			std::optional<uint32_t> presentFamily;

			bool isComplete() {
				return graphicsFamily.has_value() && presentFamily.has_value();
			}
		};

	public:
		vk::PhysicalDevice m_physical_device;
		vk::PhysicalDeviceProperties m_physical_device_properties;

		const Instance* m_instance;
		const Window* m_window;

		int m_score;

	public:
		PhysicalDevice(const Instance* instance, const Window* window) : m_window(window), m_instance(instance) {
			pickPhysicalDevice();
		}

		~PhysicalDevice() {}

		QueueFamilyIndices findQueueFamilies() const {
			return findQueueFamilies(m_physical_device, m_window);
		}

	private:
		static QueueFamilyIndices findQueueFamilies(vk::PhysicalDevice device, const Window* window) {
			QueueFamilyIndices queue_family_indices = {};

			std::vector<vk::QueueFamilyProperties> available_queue_families = device.getQueueFamilyProperties();
			for (size_t i = 0; i < available_queue_families.size(); i++) {
				// graphics queue
				if (available_queue_families[i].queueFlags & vk::QueueFlagBits::eGraphics) {
					queue_family_indices.graphicsFamily = i;
				}

				// surface support
				vk::Bool32 presentSupport = false;
				device.getSurfaceSupportKHR(i, window->m_surface, &presentSupport);
				if (presentSupport) {
					queue_family_indices.presentFamily = i;
				}
			}

			return queue_family_indices;
		}

		bool isDeviceSuitable(vk::PhysicalDevice device) {
			QueueFamilyIndices queue_family_indices = findQueueFamilies(device, m_window);

			bool features = hasRequiredFeatures(device);
			bool properties = hasRequiredProperties(device);
			bool extensions = hasRequiredExtensions(device, m_window->m_surface);

			return queue_family_indices.isComplete() && extensions && features;
		}

		int rateDeviceSuitability(vk::PhysicalDevice device) {
			// assign score to all GPUs
			int score = 1;

			if (!isDeviceSuitable(device)) return 0;

			// Discrete GPUs have a significant performance advantage
			if (device.getProperties().deviceType == vk::PhysicalDeviceType::eIntegratedGpu) { // prefer integrated for now
				score += 1;
			}

			return score;
		}

		vk::PhysicalDevice bestPhysicalDevice(const std::vector<vk::PhysicalDevice>& devices) {
			// Use an ordered map to automatically sort candidates by increasing score
			std::multimap<int, VkPhysicalDevice> candidates;
			for (const auto& device : devices) {
				int score = rateDeviceSuitability(device);
				candidates.insert(std::make_pair(score, device));
			}

			// count all suitable GPUs
			size_t suitable = 0;
			for (auto gpu : candidates) {
				if (gpu.first > 0) suitable++;
			}

			// check if any
			if (suitable == 0) {
				errorLog("No suitable GPUs");
			}

			// check if the best GPU is even suitable
			auto best = candidates.rbegin();
			if (best->first > 0) {
				return best->second;
			}
		}

		void pickPhysicalDevice() {
			// all GPUs with Vulkan support
			uint32_t deviceCount = 0;
			
			std::vector<vk::PhysicalDevice> devices = m_instance->m_instance.enumeratePhysicalDevices();
			if (devices.size() == 0) {
				errorLog("No GPUs with Vulkan support");
			}

			// select GPU
			m_physical_device = bestPhysicalDevice(devices);
			m_physical_device_properties = m_physical_device.getProperties();
		}

	};

}