...
namespace oe::gui
{
	...
	/* load example: [ // widget array
		{ // widget
			...
			"text_options": {
				...
				"align": [ 0.0, 0.0 ],
				"resolution": 16,
				"scale": [ 16.0, 16.0 ], // per axis scaling
			...
			// checkbox type specifics (defaults set)
			"initial": false,
			"color_back": [ 0.25, 0.25, 0.25, 1.0 ],
	...
	private:
		...
		std::unordered_map<std::string_view, std::pair<widgets, std::shared_ptr<oe::gui::Widget>>> widget_map{};
		std::unordered_map<std::string_view, oe::graphics::Sprite> sprite_map{};
		oe::graphics::Sprite sprite_empty{};

	public:
		JGUI(const oe::graphics::Window& window);

		// loaders
		
		std::shared_ptr<Widget> load_widget(const nlohmann::json& widget, std::string_view parent = {});

		inline void load(const nlohmann::json& json_data, std::string_view parent = {})
		{
			base_json = json_data;

			// iterate widget array
			for (const auto& widget : json_data)
				load_widget(widget, parent);
		}

		template<typename T>
		std::shared_ptr<T> load_widget_T(const nlohmann::json& widget, std::string_view parent = {})
		{
			return std::dynamic_pointer_cast<T>(load_widget(widget, parent));
		}
		...
		{
			return widget_map.at(path).second;
		}
