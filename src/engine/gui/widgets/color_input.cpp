...
namespace oe::gui
{
	...
	VecInfo<SliderInput, vec_size> create_vec_slider_info(const ColorInputInfo& color_input_info)
	{
		const int slider_height = color_input_info.widget_info.size.y;
		const int slider_width = color_input_info.widget_info.size.x - 55;

		VecInfo<SliderInput, vec_size> info;
	...
	VecInfo<NumberInput, vec_size> create_vec_number_info(const ColorInputInfo& color_input_info)
	{
		const int slider_height = color_input_info.widget_info.size.y;
		const int slider_width = color_input_info.widget_info.size.x - 55;

		VecInfo<NumberInput, vec_size> info;
	...
	ColorInput::ColorInput(Widget* parent, GUI& gui_manager, value_t& value_ref, const ColorInputInfo& color_input_info)
		: Widget(parent, gui_manager, color_input_info.widget_info)
		, m_color_input_info(color_input_info)
		, m_value(value_ref)
	{
		auto use_event = [this](const SliderInputUseEvent& e){
		...
		auto hover_event = [this](const SliderInputHoverEvent&){
		...
		if(m_color_input_info.primary_input == input_type::slider)
		{
			...
			for (size_t i = 0; i < 4; i++)
				m_cg_vec_hover[i].connect<SliderInputHoverEvent>(vec->m_elements[i]->dispatcher, hover_event);
		...
		if (m_color_input_info.popup_color_picker)
		{
			...
			m_preview_button = create<Button>(b_info);
	...
	void ColorInput::virtual_toggle(bool enabled)
	{
		...
		if(enabled)
		{
			m_cg_render.connect<GUIRenderEvent, &ColorInput::on_render>(m_gui_manager.dispatcher, this);
			m_cg_mouse_button.connect<MouseButtonEvent, &ColorInput::on_mouse_button>(m_gui_manager.dispatcher, this);
			m_cg_cursor_pos.connect<CursorPosEvent, &ColorInput::on_cursor_pos>(m_gui_manager.dispatcher, this);
		}
		else
		{
