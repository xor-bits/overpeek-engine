...
void Gui::render(graphics::Renderer *renderer_blurred, graphics::Renderer *renderer) {
	...
	if (Game::advancedDebugMode) {
		...
		renderer->renderText(-Game::getWindow()->getAspect() + (GUI_FRAME_LOGGER_BAR_WIDTH * (GUI_FRAME_LOGGER_SIZE / 2.0 - 1)),
			-0.02, 0.05, 0.05, 0.0, "frame micro-s: " + std::to_string(m_slowest_frame), glm::vec4(1.0, 1.0, 1.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP);
		...
		renderer->renderText(Game::getWindow()->getAspect() - (GUI_FRAME_LOGGER_BAR_WIDTH * (GUI_FRAME_LOGGER_SIZE / 2.0 - 1)),
			-0.02, 0.05, 0.05, 0.0, "update micro-s: " + std::to_string(m_slowest_update), glm::vec4(1.0, 1.0, 1.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP);
	...
	if (Game::paused) {
		...
		renderer->renderText(0.003, -0.003 - 0.75, textScale, textScale, 0, text, glm::vec4(0.0, 0.0, 0.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
		renderer->renderText(-0.003, 0.003 - 0.75, textScale, textScale, 0, text, glm::vec4(1.0, 1.0, 1.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
		...
		textScale = 0.05;
		text = "RESUME";
		renderer->renderText(0.003, -0.003 - 0.5, textScale, textScale, 0, text, glm::vec4(0.0, 0.0, 0.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
		renderer->renderText(-0.003, 0.003 - 0.5, textScale, textScale, 0, text, glm::vec4(1.0, 1.0, 1.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);

		textScale = 0.05;
		text = "SAVE AND EXIT";
		renderer->renderText(0.003, -0.003 - 0.35, textScale, textScale, 0, text, glm::vec4(0.0, 0.0, 0.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
		renderer->renderText(-0.003, 0.003 - 0.35, textScale, textScale, 0, text, glm::vec4(1.0, 1.0, 1.0, 1.0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
	}
}
