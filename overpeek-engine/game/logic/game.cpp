...
void Game::init(graphics::Shader *shader, graphics::Window * window, logic::GameLoop *loop) {
	...
	TextureManager::loadTexture("recourses/sand.png", GL_RGB, 2);
	TextureManager::loadTexture("recourses/grass.png", GL_RGB, 3);
	TextureManager::loadTexture("recourses/mud.png", GL_RGB, 4);
	TextureManager::loadTexture("recourses/tree.png", GL_RGBA, 5);
...
void Game::render() {
	for (int x = 0; x < RENDER_DST; x++)
	{
		for (int y = 0; y < RENDER_DST; y++)
		{
			if (m_region[x][y] != nullptr) m_region[x][y]->render(-m_player->getX() * TILE_SIZE, -m_player->getY() * TILE_SIZE);
		}
	}
	m_player->render();
